...
function MyTeams() {
  const getCardImage = (eventName) => {
    return (
      <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-xl border border-emerald-700/20 bg-slate-100 text-center shadow-[0_0_0_1px_rgba(16,185,129,0.18)] dark:border-emerald-500/30 dark:bg-black">
        ...
        <div className="flex flex-col gap-4">
          {teamsList.map((team) => {
            return (
              <button
                key={team.id}
                className="group w-full rounded-2xl border border-emerald-700/20 bg-white p-4 text-left shadow-[0_8px_20px_rgba(15,23,42,0.08)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,23,42,0.12)] dark:border-emerald-500/30 dark:bg-[#0b0b0b] dark:shadow-[0_8px_24px_rgba(0,0,0,0.32)] dark:hover:shadow-[0_12px_28px_rgba(0,0,0,0.4)]"
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="rounded-full border border-emerald-700/20 bg-slate-100 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-700 dark:border-emerald-500/30 dark:bg-slate-900 dark:text-slate-200">
                        {team.role}
                      </span>
                    </div>

                    <div className="grid gap-2 text-sm text-slate-700 dark:text-slate-300 sm:grid-cols-2">
                      <div className="flex items-center gap-2 rounded-lg border border-emerald-700/20 bg-slate-50 px-3 py-2 dark:border-emerald-500/30 dark:bg-slate-950/80">
                      ...
                      <div className="flex items-center gap-2 rounded-lg border border-emerald-700/20 bg-slate-50 px-3 py-2 dark:border-emerald-500/30 dark:bg-slate-950/80 sm:col-span-2">
                        <Users className="h-4 w-4 text-emerald-500 dark:text-emerald-400" />
